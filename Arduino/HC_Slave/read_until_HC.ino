...
//This function goes and finds a keyword received from the HC
...
// byte mode - if '1', this function will save every byte received to 'scratch_data_from_HC[]' with length of 'scratch_length' - useful for parsing between keywords
boolean read_until_HC(const char keyword1[], int key_size, int timeout_val, byte mode){
 ...
 for(byte i=0; i<key_size; i++){//we only need a buffer as long as the keyword
            ...
            //timing control
            while(!HC.available()){//wait until a new byte is sent down from the HC - good way to keep in lock-step with the serial port
    ...
    data_in[i]=HC.read();// save the byte to the buffer 'data_in[]

    if(mode==1){//this will save all of the data to the scratch_data_from
      scratch_data_from_HC[scratch_length]=data_in[i];//starts at 1
      scratch_data_from_HC[0]=scratch_length;// [0] is used to hold the length of the array
...
//THE BUFFER IS FULL, SO START ROLLING NEW DATA IN AND OLD DATA OUT
  while(1){//stay in here until the keyword found or a timeout occurs
     ...
     //this check is here, just in case the first thing out of the HC was the keyword, meaning the buffer was actually filled with the keyword
    ...
    for(byte i=0; i<(key_size-1); i++){// keysize-1 because everthing is shifted over - see next line
      data_in[i]=data_in[i+1];// so the data at 0 becomes the data at 1, and so on.... the last value is where we'll put the new data
           ...
           //timing control
            while(!HC.available()){// same thing as done in the buffer
    ...
    data_in[key_size-1]=HC.read();//save the new data in the last position in the buffer
   
      if(mode==1){//continue to save everything if thsi is set
      scratch_data_from_HC[scratch_length]=data_in[key_size-1];
      scratch_data_from_HC[0]=scratch_length;
      scratch_length++;
    }//mode 1
